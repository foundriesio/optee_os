...
static const struct crypto_cipher_ops se05x_des_ecb_ops;

static struct se05x_des_ecb_ctx *to_des_ecb_ctx(struct crypto_cipher_ctx *ctx)
{
	assert(ctx && ctx->ops == &se05x_des_ecb_ops);

	return container_of(ctx, struct se05x_des_ecb_ctx, ctx);
}

static TEE_Result se05x_des_ecb_init(struct crypto_cipher_ctx *ctx,
...
{
	struct se05x_des_ecb_ctx *c = to_des_ecb_ctx(ctx);
...
static TEE_Result se05x_des_ecb_update(struct crypto_cipher_ctx *ctx,
...
static void se05x_des_ecb_final(struct crypto_cipher_ctx *ctx)
{
	struct se05x_des_ecb_ctx *c = to_des_ecb_ctx(ctx);
...
static void se05x_des_ecb_free_ctx(struct crypto_cipher_ctx *ctx)
{
	free(to_des_ecb_ctx(ctx));
}

static void se05x_des_ecb_copy_state(struct crypto_cipher_ctx *dst_ctx,
				     struct crypto_cipher_ctx *src_ctx)
{
	struct se05x_des_ecb_ctx *src = to_des_ecb_ctx(src_ctx);
	struct se05x_des_ecb_ctx *dst = to_des_ecb_ctx(dst_ctx);
...
static const struct crypto_cipher_ops se05x_des_ecb_ops = {
	.init = se05x_des_ecb_init,
	.update = se05x_des_ecb_update,
	.final = se05x_des_ecb_final,
	.free_ctx = se05x_des_ecb_free_ctx,
	.copy_state = se05x_des_ecb_copy_state,
};

TEE_Result crypto_des_ecb_alloc_ctx(struct crypto_cipher_ctx **ctx_ret)
{
	struct se05x_des_ecb_ctx *c = NULL;

	c = calloc(1, sizeof(*c));
	if (!c)
		return TEE_ERROR_OUT_OF_MEMORY;

	c->ctx.ops = &se05x_des_ecb_ops;
	*ctx_ret = &c->ctx;
