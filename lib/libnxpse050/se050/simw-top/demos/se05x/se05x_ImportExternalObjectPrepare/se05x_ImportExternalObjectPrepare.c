...
void usage()
{
    LOG_W(
        "\n\
    Usage:\n\
    se05x_ImportExternalObjectCreate.exe \n\
        -keyid <ECDSA auth object id>\n\
        -file  <Input ECDSA keypair file>\n\
        -out   <Output file to store raw APDU>");
    return;
}
...
smStatus_t fLogTransmitBufferWriteToFile(Se05xSession_t *pwrite_apdubufferctx,
...
void getWriteOnlyKeyPolicy(uint8_t *pPolicyBuffer, size_t *pPolicyBufferLen);
...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    pse05xWriteBufferSessionCtx->fp_TXn = &fLogTransmitBufferWriteToFile;
...
static sss_status_t parse_command_line_args(int argc, const char **argv)
{
    ...
    bool output_file_parameter_passed = false;
    ...
    for (int j = 1; j < argc; j++) {
        ...
        else if (strcmp(argv[j], "-out") == 0) {
            output_file_parameter_passed = true;
            j++;
            output_filename = (char *)argv[j];
        }
    ...
    if (!output_file_parameter_passed) {
        /* Even if the output file parameter is not passed, the APDU will
         * be printed in logs. The user can manually create a file
         * if required.
         */
        LOG_W("Output file parameter not passed");
        usage();
    }
...
#if SSS_HAVE_SE05X_VER_GTE_04_04 || SSS_HAVE_FIPS
...
static sss_status_t nxECKey_HostLocal_CalculateSessionKeys(ex_sss_boot_ctx_t *pCtx)
{
    ...
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    LOG_MAU8_D("sessionEncKey", sessionEncKey, 16);
    ...
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    LOG_MAU8_D("sessionRmacKey", sessionRmacKey, 16);
...
static sss_status_t nxECKey_Calculate_Initial_Mac_Chaining_Value(ex_sss_boot_ctx_t *pCtx)
{
    ...
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    LOG_MAU8_D("Initial MCV", iniMacChaining, 16);
    ...
    uint8_t hasle)
{
    ...
    if (output_filename) {
        ...
        if (fp == NULL) {
            LOG_E("Cannot open file %s", output_filename);
            return SM_NOT_OK;
        }
        LOG_I("APDU Written to %s", output_filename);
        fwrite(gTxBuffer, 1, gTxBufferLen, fp);
        fclose(fp);
        ret = SM_OK;
    }
...
void getWriteOnlyKeyPolicy(uint8_t *pPolicyBuffer, size_t *pPolicyBufferLen)
{
    ...
    LOG_I("Policy for WriteOnly Key:");
    ...
    LOG_I("Locked = Write");
    pFile->can_Write = 1;
    LOG_I("Excluded = DELETE, READ.");
    pCommon->can_Delete = 1;
    pFile->can_Read     = 0;
