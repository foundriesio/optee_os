...
USB_RAM_ADDRESS_ALIGNMENT(16) static usb_device_dwc3_trb_struct_t s_UsbDeviceDwc3Trb[USB_DEVICE_CONFIG_DWC3][USB_DEVICE_CONFIG_ENDPOINTS * 2];
USB_RAM_ADDRESS_ALIGNMENT(8) static uint8_t s_UsbDeviceDwc3EventBuffer[USB_DEVICE_CONFIG_DWC3][USB_DEVICE_CONFIG_DWC3_EVENT_COUNT][USB_DEVICE_DWC3_EVENT_BUFFER_COUNT * USB_DEVICE_DWC3_EVENT_BUFFER_SIZE];
USB_RAM_ADDRESS_ALIGNMENT(4) static uint8_t s_UsbDeviceDwc3SetupBuffer[USB_DEVICE_CONFIG_DWC3][USB_SETUP_PACKET_SIZE];
USB_RAM_ADDRESS_ALIGNMENT(4) static uint8_t s_UsbDeviceDwc3ScratchpadBuffer[USB_DEVICE_CONFIG_DWC3][USB_DEVICE_CONFIG_DWC3_SCRATCHPAD_BUFFER_SIZE];
    ...
    */
    ...
    if (USB_ENDPOINT_ISOCHRONOUS == dwc3State->endpointState[index].stateUnion.stateBitField.type)
    {
        ...
        else
        {
            ...
            {
                frame = (currentframe/dwc3State->endpointState[index].interval + 1)*dwc3State->endpointState[index].interval;
            }
        }
    }
...
static void USB_DeviceDwc3SetDefaultState(usb_device_dwc3_state_struct_t *dwc3State)
{
    usb_device_endpoint_init_struct_t epOutInit = {
        dwc3State->endpointState[0].stateUnion.stateBitField.maxPacketSize, 0U, USB_ENDPOINT_CONTROL, 0U,
    };
    usb_device_endpoint_init_struct_t epInInit = {
        dwc3State->endpointState[1].stateUnion.stateBitField.maxPacketSize, 0x80U, USB_ENDPOINT_CONTROL, 0U,
    };
                                               ...
                                               usb_device_endpoint_init_struct_t *epInit)
{
    ...
    if ((index == 0) && (!dwc3State->endpointState[index].stateUnion.stateBitField.configured))
    {
        ...
        USB_DeviceDwc3EndpointSpecificCommand(dwc3State, &command);
        /*Issue a DEPSTARTCFG command with DEPCMD0.XferRscIdx set to 2 to re-initialize the transfer resource allocation.*/
    ...
    if ((USB_SPEED_SUPER == dwc3State->speed) || (USB_SPEED_HIGH == dwc3State->speed))
    {
        if (epInit->interval)
        {
            /*The bInterval value is reported in the endpoint descriptor. When the core is operating in Full-Speed mode, this field must be set to 0.*/
