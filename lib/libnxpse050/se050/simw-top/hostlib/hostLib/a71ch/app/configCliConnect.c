 ...
 *
 * @par Description
 * Command line handling 'connect' entry
 */
...
int a7xConfigCliCmdConnect(int argc, char **argv, U16 *sw)
{
    ...
    if (strcmp(argv[argCurrent], "close") == 0) {
        return a7xConfigCmdConnectClose(sw);
    }
    else if (strcmp(argv[argCurrent], "open") ==0 ) {
        return a7xConfigCmdConnectOpen(sw);
    }
    else {
