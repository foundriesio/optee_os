...
void mfa_process_testIoTPreUpgrade()
{
    ...
    status = sss_key_object_init(&keyPair, &gfeature_app_mals_boot_ctx.ks);
    ...
    status = sss_asymmetric_context_init(&asymCtx,
        &gfeature_app_mals_boot_ctx.session,
        &keyPair,
        algorithm,
        mode);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    status = sss_key_store_generate_key(
        &gfeature_app_mals_boot_ctx.ks, &keyPair, 256, 0);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
...
void mfa_process_testIoTPostUpgrade()
{
    ...
    status = sss_key_store_get_key(&gfeature_app_mals_boot_ctx.ks,
        &keyPair,
        pbKey,
        &pbKeyBytetLen,
        &pbKeyBitLen);
