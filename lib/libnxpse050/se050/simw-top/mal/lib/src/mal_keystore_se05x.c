...
#define IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_MAJOR (0)
#define IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_MINOR (0)
#define IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_PATCH (1)
#define IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION                                                                   \
    (((IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_MAJOR * 256) + IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_MINOR) * 256 + \
        IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION_PATCH)

const iot_agent_keystore_interface_t iot_agent_keystore_mal_se05x_interface = {&iot_agent_keystore_mal_se05x_destroy,
    &iot_agent_keystore_mal_se05x_open_session,
    &iot_agent_keystore_mal_se05x_close_session,
    {
        &iot_agent_keystore_mal_se05x_get_endpoint_info,
        &iot_agent_keystore_mal_se05x_handle_request,
    }};
    ...
    iot_agent_keystore_t *keystore, int identifier, iot_agent_keystore_mal_se05x_context_t *keystore_context)
{
    ...
    keystore->iface                 = iot_agent_keystore_mal_se05x_interface;
...
bool iot_agent_keystore_mal_se05x_get_endpoint_info(void *context, void *endpoint_information)
{
    ...
    info->version                     = IOT_AGENT_KEYSTORE_MAL_SE05x_VERSION;
    return true;
}
...
{
    mal_agent_ctx_t *mal_agent_context = NULL;
    if (context != NULL) {
        mal_agent_context = ((iot_agent_keystore_mal_se05x_context_t *)context)->mal_agent_context;
        if (mal_agent_context == NULL) {
            // Check NULL pointer.
            IOT_AGENT_ERROR("MAL agent context pointer is NULL");
            return false;
        }
    }
    else {
    ...
    if (message_type == nxp_iot_HostControlCmdRequest_fields) {
        ...
        if (mal_agent_context->skip_next_commands) {
            /** Only for MALS agent. Skip current command if some error happens in former command*/
        ...
        if (request.has_hostControlCmd) {
            if (request.hostControlCmd == nxp_iot_HostControlCmdRequest_eControlCmd_RESET_SE) {
                if (mal_agent_reset_se(mal_agent_context) != kStatus_MAL_Success)
                    return false;
            }
            else
                return false;
        ...
        mal_agent_context->status_word = MAL_AGENT_STATUS_WORD_HOST_CMD_OK;
    }
        ...
        if (SW_OK != smCom_TransceiveRaw(mal_agent_context->pS05x_Ctx->conn_ctx,
                         read_buffer.buf,
                         (U16)read_buffer.len,
                         write_buffer.buf,
                         &len)) {
            IOT_AGENT_ERROR("MALS Agent Communitcation Failure");
            mal_agent_context->status_word        = MAL_AGENT_STATUS_WORD_COM_FAILURE;
            mal_agent_context->skip_next_commands = true;
            return false;
        }
        ...
        if (len >= 2) {
            ...
            */
            ...
            if (status_word == ERR_COMM_ERROR) {
                ...
                mal_agent_context->status_word = status_word;
            }

            if (verify_return_value(status_word, &expectation)) {
                mal_agent_context->skip_next_commands = false;
                mal_agent_context->status_word        = MAL_AGENT_STATUS_WORD_USER_DEFINE_SUCCESS;
            }
