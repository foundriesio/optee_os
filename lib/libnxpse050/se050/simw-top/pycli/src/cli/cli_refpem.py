...
@pass_context
def pair(cli_ctx, keyid, filename, format, password):  # pylint: disable=function-redefined, redefined-builtin
    ...
    try:
        keyid = int(keyid, 16)
        if password == "" and \
           (filename.endswith('.pfx') or filename.endswith('.p12') or format == "PKCS12"):
        ...
        try:
            ...
            status = func_timeout.func_timeout(TIME_OUT, refpem_obj.do_ecc_refpem_pair,
                                               (keyid, filename, format, password))
...
@click.option('--password', default="", type=str, help="Password used for PKCS12 format.")
@pass_context
def pub(cli_ctx, keyid, filename, format, password):  # pylint: disable=redefined-builtin
