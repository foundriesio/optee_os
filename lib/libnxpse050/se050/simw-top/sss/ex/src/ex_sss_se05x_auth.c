    ...
    NXSCP03_AuthCtx_t *pAuthCtx, ex_SE05x_authCtx_t *pEx_auth, sss_key_store_t *host_k)
{
    ...
    /* Set ENC Static Key */
    status = sss_host_key_store_set_key(host_k, &pStatic_ctx->Enc, APPLET_KEY, 16, 16 * 8, NULL, 0);
    if (status != kStatus_SSS_Success) {
        return status;
    }
    ...
    /* Set MAC Static Key */
    status = sss_host_key_store_set_key(host_k, &pStatic_ctx->Mac, APPLET_KEY, 16, 16 * 8, NULL, 0);
    if (status != kStatus_SSS_Success) {
        return status;
    }
    ...
    /* Set DEK Static Key */
    status = sss_host_key_store_set_key(host_k, &pStatic_ctx->Dek, APPLET_KEY, 16, 16 * 8, NULL, 0);
    if (status != kStatus_SSS_Success) {
        return status;
    }
    ...
    sss_object_t *keyObject, uint32_t keyId, sss_key_store_t *host_ks)
{
    ...
    status = sss_host_key_object_allocate_handle(
        keyObject, keyId, kSSS_KeyPart_Default, kSSS_CipherType_AES, 16, kKeyObject_Mode_Persistent);
    return status;
}
#endif // SSSFTR_SE05X_AuthSession
