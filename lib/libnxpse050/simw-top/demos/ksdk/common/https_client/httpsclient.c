...
#define GlobalSignRootCA_R2_PEM \
    "-----BEGIN CERTIFICATE-----\r\n\
...
const char *HTTPS_SERVER_NAME = "purnank-hrd.appspot.com";
//#define GET_REQUEST "GET /media/uploads/mbed_official/hello.txt HTTP/1.0\r\n\r\n"
#define GET_REQUEST "GET /c HTTP/1.0\r\nHost: purnank-hrd.appspot.com\r\nConnection: keep-alive\r\n\r\n"
#define POST_REQUEST                                                                          \
    "POST /c HTTP/1.0\r\nHost: purnank-hrd.appspot.com\r\nContent-Length: 13\r\nConnection: " \
    "keep-alive\r\n\r\nname=testname"
const char *HTTPS_SERVER_PORT = "443";
unsigned char https_buf[1024];
const char https_root_ca_crt[] = GlobalSignRootCA_R2_PEM; /* SHA1 or SHA256 */
...
int write_request(const char *REQUEST)
{
    /*
    ...
    int len = sprintf((char *)https_buf, REQUEST);
...
int https_client_tls_init()
{
    ...
    if ((ret = mbedtls_hmac_drbg_seed(&(tlsDataParams.hmac_drbg),
             md_info,
             mbedtls_entropy_func,
             &(tlsDataParams.entropy),
    ...
    ret = mbedtls_x509_crt_parse(
        &(tlsDataParams.cacert), (const unsigned char *)https_root_ca_crt, sizeof(https_root_ca_crt));
    if (ret < 0) {
    ...
    ret = mbedtls_x509_crt_parse(
        &(tlsDataParams.clicert), (const unsigned char *)mbedtls_test_cli_crt, mbedtls_test_cli_crt_len);
    if (ret != 0) {
    ...
    ret = mbedtls_pk_parse_key(
        &(tlsDataParams.pkey), (const unsigned char *)mbedtls_test_cli_key, mbedtls_test_cli_key_len, NULL, 0);
    if (ret != 0) {
    ...
    if ((ret = mbedtls_ssl_config_defaults(
             &(tlsDataParams.conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)) !=
        0) {
