...
// Non-secure world
/* To call RNG functions from Non-Secure world, we have to
 * define these as a part of veneer table.
 */
#include "sec_rng_defs.h"
#else
// Secure World
/* RNG can be called from secure world directly */
#define SEC_RNG_Init RNG_Init
#define SEC_RNG_GetRandomData RNG_GetRandomData
#endif

static void ksdk_mbedtls_rng_init(void)
{
#if defined(FSL_FEATURE_SOC_TRNG_COUNT) && (FSL_FEATURE_SOC_TRNG_COUNT > 0)
#if defined(TRNG)
#define TRNG0 TRNG
...
#define TRNG0 TRNG
    ...
    result = SEC_RNG_GetRandomData(RNG, output, len);
#endif
