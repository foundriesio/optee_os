...
phStatus_t phOsal_ThreadChangePrio(phOsal_Thread_t * threadHandle, uint32_t newPrio)
{
    ...
    status = pthread_setschedparam((pthread_t)(*threadHandle), 0, &sSchedParam);
    if (status != PH_OSAL_SUCCESS)
    {
...
phStatus_t phOsal_ThreadDelay(phOsal_Ticks_t ticksToSleep)
{
    ...
    sTimeSpecReq.tv_sec += sTimeSpecReq.tv_nsec / (1000 * 1000 * 1000);
    sTimeSpecReq.tv_nsec %= (1000 * 1000 * 1000);
...
phStatus_t phOsal_EventDelete(phOsal_Event_t * eventHandle)
{
    ...
    gdwEventBitMap &= ~(1 << (((phOsal_EventObj_t *)(eventHandle))->dwEventIndex));
...
phStatus_t phOsal_SemPend(phOsal_Semaphore_t * semHandle, phOsal_TimerPeriodObj_t timePeriodToWait)
{
    ...
    else
    {
        ...
        else
        {
            ...
            sTimerSpec.tv_nsec += (timePeriodToWait.period % 1000) * 1000 * 1000; /**< Most of the cases this evaluates to zero. */
        }
...
phStatus_t phOsal_MutexLock(phOsal_Mutex_t * mutexHandle, phOsal_TimerPeriodObj_t timePeriodToWait)
{
    ...
    else
    {
        ...
        else
        {
            ...
            tspec.tv_nsec += (timePeriodToWait.period % 1000) * 1000 * 1000; /**< Most of the cases this evaluates to zero. */
        }
...
{
    phStatus_t status = (PH_OSAL_ERROR | PH_COMP_OSAL);
#ifndef NXPBUILD__PH_OSAL_OSX
...
phStatus_t phOsal_TimerStart(phOsal_Timer_t *timerHandle)
{
    ...
    if (pTimerObj->opt == OS_TIMER_OPT_PERIODIC)
    {
        ...
        else
        {
            ...
            sTimerSpec.it_interval.tv_nsec = (pTimerObj->timePeriod.period % 1000) * 1000 * 1000; /**< Most of the cases this evaluates to zero. */
        }

        /* Make sure sTimerSpec.it_interval is valid */
        sTimerSpec.it_interval.tv_sec += sTimerSpec.it_interval.tv_nsec / (1000 * 1000 * 1000);
        sTimerSpec.it_interval.tv_nsec %= (1000 * 1000 * 1000);
    }
    else
    {
    ...
    else
    {
        ...
        sTimerSpec.it_value.tv_nsec = (pTimerObj->timePeriod.period % 1000) * 1000 * 1000; /**< Most of the cases this evaluates to zero. */
    }

    /* Make sure sTimerSpec.it_value is valid */
    sTimerSpec.it_value.tv_sec += sTimerSpec.it_value.tv_nsec / (1000 * 1000 * 1000);
    sTimerSpec.it_value.tv_nsec %= (1000 * 1000 * 1000);
