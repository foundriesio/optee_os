 ...
 *
 * \brief This component implements the OSAL APIs used by the Nxp Reader Library.
 * All these functions are used by the NXP reader library internally.
 *
 * If compiling only the library NxpNfcRdLib, the file NxpNfcRdLib/types/ph_NxpBuild_Lpc.h should define the Osal
 * used by the User.
 *
 *
 * API Changes
 ...
 * | Old API                                | Changed API
 * |----------------------------------------|-----------------------------------------
 * | \ref phOsal_Event_Consume()            | \ref phOsal_EventClear
 * | \ref phOsal_Event_Post()               | \ref phOsal_EventPost
 * | \ref phOsal_Event_WaitAll()            | \ref phOsal_EventPend
 * | \ref phOsal_Event_WaitAny()            | \ref phOsal_EventPend
 * | \ref phOsal_Mutex_Create()             | \ref phOsal_MutexCreate
 * | \ref phOsal_Mutex_Delete()             | \ref phOsal_MutexDelete
 * | \ref phOsal_Mutex_Give()               | \ref phOsal_MutexUnLock
 * | \ref phOsal_Mutex_Take()               | \ref phOsal_MutexLock
 * | \ref phOsal_Semaphore_Create()         | \ref phOsal_SemCreate
 * | \ref phOsal_Semaphore_Delete()         | \ref phOsal_SemDelete
 * | \ref phOsal_Semaphore_Give()           | \ref phOsal_SemPost
 * | \ref phOsal_Semaphore_Take()           | \ref phOsal_SemPend
 * | \ref phOsal_TaskDelay()                | \ref phOsal_ThreadDelay
 * | \ref phOsal_Timer_Create()             | \ref phOsal_TimerCreate
 * | \ref phOsal_Timer_Delete()             | \ref phOsal_TimerDelete
 * | \ref phOsal_Timer_Start()              | \ref phOsal_TimerStart
 * | \ref phOsal_Timer_Stop()               | \ref phOsal_TimerStop
 *
 *
 * The following internal APIs have been refactored and re-implemented
 * for generic portability
 *
 * | Old Internal API                       | Status
 * |----------------------------------------|-----------------------------------------
 * | \ref phOsal_Event_Deinit()             | Not Supported
 * | \ref phOsal_Event_GetDetail()          | Not Supported
 * | \ref phOsal_Event_GetEventHandle()     | Not Supported
 * | \ref phOsal_Event_Init()               | Not Supported
 * | \ref phOsal_Event_PostwithDetail()     | \see phOsal_EventPost
 * | \ref phOsal_Event_SetDetail()          | Not Supported

 * | New APIs                                |
 * |-----------------------------------------|
 * | \ref phOsal_StartScheduler              |
...
typedef void (*pphOsal_StartFunc_t)(void*);
 ...
 * @retval #PH_ERR_INVALID_PARAMETER    Parameter passed is invalid.
 * @retval #PH_ERR_RESOURCE_ERROR       Resource not available incase of Static allocation.
 */
 ...
 * @retval #PH_ERR_SUCCESS              Operation successful.
 * @retval #PH_ERR_INVALID_PARAMETER    Parameter passed is invalid.
 * @retval #PH_ERR_IO_TIMEOUT           Desired events didn't occur during the wait time(ticksToWait).
 */
 ...
 * @retval #PH_ERR_UNSUPPORTED_COMMAND  Command not supported.
 * @retval #PH_ERR_INVALID_PARAMETER    Parameter passed is invalid.
 * @retval #PH_ERR_IO_TIMEOUT           Time expired without semaphore becoming available.
 */
 ...
 * @retval #PH_ERR_UNSUPPORTED_COMMAND  Command not supported.
 * @retval #PH_ERR_INVALID_PARAMETER    Parameter passed is invalid.
 * @retval #PH_ERR_IO_TIMEOUT           Time expired without mutex getting locked.
 */
 ...
 *
 * @param[in]  timerHandle          Newly created Timer Handle.
 * @param[out]  dwGetElapsedTime    Elapsed Time value(ms)
 *
 * @return Status of the API
 * @retval #PH_ERR_SUCCESS              Operation successful.
 * @retval #PH_ERR_INVALID_PARAMETER    Parameter passed is invalid.
