...
int network_openssl_init(openssl_network_context_t* network_ctx) {
	...
	network_ctx->engine_session_connected = true;
...
int network_openssl_engine_session_disconnect(openssl_network_context_t* network_ctx) {
	...
	if (!network_ctx->engine_session_connected)
		return NETWORK_STATUS_OK;
	...
	network_ctx->engine_session_connected = false;
...
void network_print_X509_NAME(X509_NAME* name) {
	if (name == NULL) {
		printf("NULL");
		return;
	}
