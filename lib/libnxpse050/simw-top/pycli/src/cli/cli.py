...
@click.option('--auth_type', default='None', type=click.Choice(list(AUTH_TYPE_MAP.keys())),
              help="Authentication type. Default is \"None\". Can be one of \"None, UserID, "
                   "ECKey, AESKey, PlatformSCP\"")
@pass_context
def connect(cli_ctx, subsystem, method, port_name, auth_type):
    """ Open Session.\n

    subsystem = Security subsystem is selected to be used. Can be one of "mbedtls, a71ch, se050,
    a71cl, openssl"\n
...
@pass_context
def sign(cli_ctx, keyid, certificate, filename, informat, outformat, hashalgo):
    """ Sign Operation \n
        keyid = 32bit Key ID. Should be in hex format. Example: 20E8A001 \n

        certificate = Input data to sign. can be raw data (DER format) or in file.
        For file, by default filename with extension .pem and .cer considered as PEM format
        and others as DER format.\n

        filename = File name to store signature data.
...
@click.argument('keyid', type=str, metavar='keyid')
@click.argument('certificate', type=str, metavar='certificate')
@click.argument('signature', type=str, metavar='signature')
@click.option('--format', default='',
              help="certificate and signature file format. TEXT can be \"DER\" or \"PEM\"")
# pylint: disable=too-many-arguments
@click.option('--hashalgo', default='',
              help='Hash algorithm. TEXT can be one of \"SHA1, SHA224, SHA256, SHA384, '
                   'SHA512, \nRSASSA_PKCS1_V1_5_SHA1, RSASSA_PKCS1_V1_5_SHA224, \n'
...
# pylint: enable=too-many-arguments
@pass_context
def verify(cli_ctx, keyid, certificate, signature, format, hashalgo):  # pylint: disable=redefined-builtin
    """ verify operation \n
        keyid = 32bit Key ID. Should be in hex format. Example: 20E8A001 \n

        certificate = Input data for verification. Can be in file or raw data \n
        For file, by default filename with extension .pem and .cer considered as PEM format and
        others as DER format. \n

        filename = signature data file for verification.
        By default filename with extension .pem in PEM format and others in DER format.
    """
    try:
        keyid = int(keyid, 16)
        ...
        status = func_timeout.func_timeout(TIME_OUT, verify_obj.do_verification,
                                           (keyid, certificate, signature, format, hashalgo))
    except func_timeout.FunctionTimedOut as timeout_exc:
        log_traceback(cli_ctx, timeout_exc.getMsg())
        status = apis.kStatus_SSS_Fail
    except Exception as exc:  # pylint: disable=broad-except
