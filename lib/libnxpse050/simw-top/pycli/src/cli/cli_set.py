...
def aes(cli_ctx, keyid, key):
    ...
    try:
        ...
        session_open(cli_ctx)
        set_object = Set(cli_ctx.session)
        status = func_timeout.func_timeout(TIME_OUT, set_object.do_set_sym_key, (keyid, key, None))
    except func_timeout.FunctionTimedOut as timeout_exc:
        log_traceback(cli_ctx, timeout_exc.getMsg())
        status = apis.kStatus_SSS_Fail
    except Exception as exc:  # pylint: disable=broad-except
...
@pass_context
def cert(cli_ctx, keyid, key, format):  # pylint: disable=redefined-builtin
    """ Set Certificate \n
...
@pass_context
def pub(cli_ctx, keyid, key, format):  # pylint: disable=function-redefined, redefined-builtin
    """ Set ECC Public Keys \n
...
@click.option('--format', default='',
              help="Input key format. TEXT can be \"DER\" or \"PEM\"")
@pass_context
def pair(cli_ctx, keyid, key, format):  # pylint: disable=function-redefined, redefined-builtin
    """ Set ECC Key pair \n
