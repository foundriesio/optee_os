...
def parse_in_args():
    ...
    parser.add_argument(
        '--subsystem',
        default="se050",
        help='Supported subsystem => ``se050``, ``a71ch``. Default: ``se050``')

    args = parser.parse_args()

    if args.subsystem not in ["se050", "a71ch"]:
...
def main():
    ...
    for ec_curve in ec_curves:
        print(ec_curve)
        run("%s openssl_provisionEC.py --key_type %s --connection_type %s --connection_data %s --subsystem %s" %
            (python_exe, ec_curve, args.connection_type, args.connection_data, args.subsystem))
        run("%s openssl_Ecdh.py --key_type %s --connection_data %s" % (python_exe, ec_curve, args.connection_data))
        run("%s openssl_EccSign.py --key_type %s --connection_data %s" % (python_exe, ec_curve, args.connection_data))
